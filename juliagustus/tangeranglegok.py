...
# Data input untuk KAB TANGGERANG LEGOK CIKASUNGKA
data = {
    'NO': [1, 2, 3, 4, 5, 6, 7, 8, 9],
    'NAMA': [
        'SUHARDIANSYAH', 'DEA MAULIDAH', 'M AZRIEL HAZNAM S', 
        'AYULIA KHAERUNNISA', 'RAHMAWATI', 'RIZAL FAUZI', 
        'FIGH TRI OKTAVIANO', 'SITI HARDIANTI', 'INE LUTFIA NOVELLA'
    ],
    'ROLE': ['SPV', 'DS', 'DS', 'DS', 'DS', 'DS', 'DS', 'DS', 'DS'],
    'Grade': [35, 25, 25, 25, 25, 25, 25, 25, 25],
    'Target': [35, 25, 25, 25, 25, 25, 25, 25, 25],
    'Sales': [0, 26, 32, 28, 5, 2, 0, 5, 2],
    'Minus/plus': [-35, 1, 7, 3, -20, -23, -25, -20, -23],
    '%': [0, 104, 128, 112, 20, 8, 0, 20, 8]
}
...
fig.suptitle('ANALISIS PENCAPAIAN SALES TEAM KAB TANGGERANG LEGOK CIKASUNGKA\nPeriode: 21 Juli - 20 Agustus', 
...
print("=" * 70)
print("ANALISIS PENCAPAIAN SALES TEAM KAB TANGGERANG LEGOK CIKASUNGKA")
...
print("   2. Coaching intensif untuk 5 sales dengan pencapaian <50%")
print("   3. Pelajari strategi dari 3 top performers (DEA, AZRIEL, AYULIA) dengan pencapaian >100%")
...
# Simpan sebagai CSV
df.to_csv('analisis_pencapaian_legok_cikasungka.csv', index=False)
print("\n💾 Data analisis telah disimpan ke: 'analisis_pencapaian_legok_cikasungka.csv'")

# Analisis tambahan - pola performa
print("\n" + "=" * 50)
print("ANALISIS POLA PERFORMA")
print("=" * 50)

# Kelompokkan berdasarkan tingkat performa
high_performers = df[df['Pencapaian_%'] >= 100]
mid_performers = df[(df['Pencapaian_%'] >= 50) & (df['Pencapaian_%'] < 100)]
low_performers = df[df['Pencapaian_%'] < 50]

print(f"High Performers (≥100%): {len(high_performers)} orang")
print(f"Mid Performers (50-99%): {len(mid_performers)} orang") 
print(f"Low Performers (<50%): {len(low_performers)} orang")

print(f"\nHigh Performers berkontribusi {high_performers['Sales'].sum()} sales ({high_performers['Sales'].sum()/total_sales*100:.1f}% dari total)")
print(f"Low Performers berkontribusi {low_performers['Sales'].sum()} sales ({low_performers['Sales'].sum()/total_sales*100:.1f}% dari total)")