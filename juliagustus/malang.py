...
# Data sales team Malang
sales_data = {
    'NAMA_DS': ['Swardana Arie Ardiansyah', 'Redy Deny Triharyanto', 'Ricky Supriadi', 
                'Sri Mawarni', 'Roy Kusdinono', 'Retno Dinafsyaratri'],
    'GRADE': ['DS', 'DS', 'DS', 'DS', 'DS', 'DS'],
    'TARGET': [25, 25, 25, 25, 25, 25],
    'SALES': [10, 3, 1, 1, 1, 0],
    'MINUS_PLUS': [-15, -22, -24, -24, -24, -25],
    'PERSENTASE': [40, 12, 4, 4, 4, 0]
}
...
print("SALES PERFORMANCE DATA - MALANG AREA")
...
fig.suptitle('ANALISIS KINERJA SALES - AREA MALANG\n21 Juli - 20 Agustus', 
             fontsize=18, fontweight='bold', y=0.98)

# 1. Pencapaian per Individu (Horizontal Bar Chart) - DIUBAH
ax1 = plt.subplot(2, 2, 1)
colors = ['#2EC4B6' for _ in df['GRADE']]  # All DS, so all green
...
for index, row in df.iterrows():
    ...
    name_parts = row['NAMA_DS'].split()
    short_name = name_parts[0] + " " + name_parts[1][0] + "." if len(name_parts) > 1 else row['NAMA_DS']
    
    print(f"{short_name} ({row['GRADE']}): {row['ACHIEVEMENT_RATE']:.1f}% - {status}")
    print(f"   Shortfall: {row['SHORTFALL']} unit dari target {row['TARGET']}")
...
print("Status: KRITIS - Perlu intervensi segera!")

print("\n5. REKOMENDASI:")
print("-" * 40)
print("• Coaching intensif untuk seluruh tim")
print("• Review strategi penjualan area Malang")
print("• Evaluasi target yang mungkin tidak realistis")
print("• Pelatihan teknik selling khusus untuk DS")
print("• Monitoring harian progress sales")
print("• Fokus pada Retno Dinafsyaratri (0% pencapaian) dan tiga DS dengan pencapaian 4%")